	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="modal-title">Add Micro Grid Controller</h4>
			</div>
								...
								<div class="form-group">
									...
									<div class="fileinput fileinput-new" data-provides="fileinput"><input type="hidden">
										...
										<div id="edit_image" style="display:none;">
											<a href="javascript:void(0);" id="remove_pic" onclick="remove_pic();">Click here to remove Uploaded Pic </a>/
...
<script>
$("#add_new_controller").click(function(){	
	$('#add_micro')[0].reset();
	$("#edit_image").hide();
	$("#edit_profile_image").hide();
	$("#modal-title").html("Add Micro Grid Controller");
	$("#is_active_div").hide();	
...
$(document).ready(function(){
	...
	var options = {
	        ...
	        complete : function(response) {
		        ...
		        //saveImageDetail(response.responseText);
	        },
	        error : function() {
	...
	$("#btn_save").click(function(){
    	...
    	if($("#add_micro").valid()){    
    		...
    		/* getLatLong(add,function(geocode){
    			  console.log(geocode)
...
function remove_pic(id){	
	...
	var microgrid_id = $("#id").val();
	$.ajax({
		url : "${pageContext.request.contextPath}/rest/micro/removepic/"+microgrid_id+"?key="+key,
		type : "get",
...
function editMicro(id){
	$.ajax({
		url : "${pageContext.request.contextPath}/rest/micro/find/"+id,
		type : "get",
		contentType : "application/json",
		dataType : "json",
		success : function(response) {		  
		  	if(response.data){
		  		...
		  		//$("#filename").val("${pageContext.request.contextPath}"+response.data['pic_url']);
		  		if(response.data['pic_url']){
		  			$("#edit_image").show();
			  		$("#view_pic").prop("href",response.data['pic_url']);
		  		}else{
		  			$("#edit_image").hide();
		  		}
		  		if (response.data['profile_img']) {
					$("#edit_profile_image").show();
					$("#view_profile_pic_mg").prop("href",response.data['profile_img']);
				} else {
					$("#edit_profile_image").hide();
				}
				...
				if(time_zone != null && time_zone != undefined){
		  		...
		  		$("#modal-title").html("Edit Micro Grid Controller");
		  		$("#is_active_div").show();
		  		$("#microgridconfig").modal("show");
		  	}
